...
AS
  ...
  V_DATA_COUNT NUMBER;
V_MID_NIGHT TIMESTAMP;
V_TMR_TWO_DAY_SHIFT CHAR(1 BYTE);
V_TMR_SHIFT_ID NUMBER;
...
V_YESTERDAY_OUT_TIME:=trunc(P_FROM_ATTENDANCE_TIME)-1;
 ...
 -- ON (A.IP_ADDRESS          =ADM.DEVICE_IP)
  ...
  IF(V_TWO_DAY_SHIFT='E' and P_TMR_TWO_DAY_SHIFT='D')THEN
    select
    ...
    AND A.ATTENDANCE_TIME <= P_TO_ATTENDANCE_TIME)
    AND A.EMPLOYEE_ID = P_EMPLOYEE_ID
    and A.ATTENDANCE_TIME>V_MID_NIGHT;
    ...
    IF (V_DATA_COUNT>1) THEN
        ...
        FROM
        (SELECT
        A.ATTENDANCE_TIME AS OUT_TIME, row_number() over (order by A.ATTENDANCE_TIME desc) as row_order
        FROM HRIS_ATTENDANCE A
        WHERE (A.ATTENDANCE_TIME >= P_FROM_ATTENDANCE_TIME
        AND A.ATTENDANCE_TIME <= P_TO_ATTENDANCE_TIME)
        AND A.EMPLOYEE_ID = P_EMPLOYEE_ID
        and A.ATTENDANCE_TIME>V_MID_NIGHT) where row_order = 2;
    END IF;
  END IF;
  ----AAA------------
